...
export default function EstimateEntryPage() {
  ...
  const handleSave = () => {
    ...
    startTransition(() => {
      ...
      toast({ title: 'Estimate Synchronized', description: 'Technical parameters and site staff logged.' });
          ...
          <CardContent className="p-10 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase text-slate-400">Primary Site Name</Label>
                <div className="relative">
                  <Mountain className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-emerald-500" />
                  <Input value={formData.nameOfSite} onChange={(e) => updateField('nameOfSite', e.target.value)} className="h-12 pl-10 border-emerald-200 bg-emerald-50/10 font-black uppercase text-emerald-700" placeholder="ENTER PRIMARY SITE" />
                </div>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase text-slate-400">File Reference</Label>
                <Input value={formData.fileNo} onChange={(e) => updateField('fileNo', e.target.value)} className="h-12 bg-slate-50 border-slate-200 rounded-xl font-bold" placeholder="MPM/GWD/..." />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase text-slate-400">Other Staff Deployed</Label>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {formData.otherStaff.map(name => (
                      <Badge key={name} variant="secondary" className="text-[9px] font-black py-0 h-6 px-2 gap-2 bg-slate-100 text-slate-600 rounded-lg">
                          {name} <X className="h-3 w-3 cursor-pointer hover:text-red-500" onClick={() => removeStaff('otherStaff', name)} />
                      </Badge>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase text-slate-400">SLR Deployment</Label>
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {formData.slr.map(name => (
                      <Badge key={name} variant="secondary" className="text-[9px] font-black py-0 h-6 px-2 gap-2 bg-blue-50 text-blue-700 border-blue-100 rounded-lg">
                        {name} <X className="h-3 w-3 cursor-pointer hover:text-red-500" onClick={() => removeStaff('slr', name)} />
                      </Badge>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase text-slate-400">CLR Deployment</Label>
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {formData.clr.map(name => (
                      <Badge key={name} variant="secondary" className="text-[9px] font-black py-0 h-6 px-2 gap-2 bg-emerald-50 text-emerald-700 border-emerald-100 rounded-lg">
                        {name} <X className="h-3 w-3 cursor-pointer hover:text-red-500" onClick={() => removeStaff('clr', name)} />
                      </Badge>
                    ))}
                  </div>
                </div>
