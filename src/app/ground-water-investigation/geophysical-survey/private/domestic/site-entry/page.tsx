...
export default function DomesticGeophysicalSurveySiteEntryPage() {
  ...
  useEffect(() => {
    if (cloudReport) {
      form.reset({
        ...defaultValues,
        ...cloudReport,
        vesData: cloudReport.vesData || defaultValues.vesData
      });
    }
  ...
  const onSubmit = (values: ReportFormValues) => {
    ...
    startTransition(() => {
      ...
      const reportData: Partial<GroundwaterReport> = {
        ...values,
        id: reportId,
      ...
      if (!isUpdate) {
        reportData.purpose = "Ground Water Investigation / Geophysical Survey / Private / Domestic";
        reportData.category = "Geophysical Survey / Private / Domestic";
        reportData.createdAt = new Date().toISOString();
        reportData.dateOfInvestigation = dateOfInvestigation;
        reportData.staffAssignment = {
      ...
      operation
        .then(() => {
          toast({ 
            title: isUpdate ? 'Report Updated' : 'Report Saved', 
            description: 'Geophysical survey data has been successfully saved.' 
          });
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" asChild>
            <Link href={id ? "/downloads" : "/ground-water-investigation/geophysical-survey/private/domestic"}>
              <ArrowLeft className="h-5 w-5" />
            </Link>
          </Button>
          <div>
            <h1 className="font-headline text-2xl font-bold tracking-tight text-foreground uppercase">
              {id ? 'Edit Vertical Electrical Sounding' : 'Vertical Electrical Sounding'}
            </h1>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="md:col-span-4">
                      ...
                      <FormControl>
                        <Input placeholder="e.g. 6M towards East from SouthWest corner of the plot" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
          ...
          <div className="flex justify-between items-center bg-muted/20 p-4 rounded-lg border border-dashed border-muted-foreground/30">
            ...
            <Button type="submit" disabled={isPending}>
              {isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}
              {id ? 'Update Survey Record' : 'Save Survey Record'}
            </Button>
          </div>
        </form>
      </Form>
