            ...
            <Table className="border-collapse min-w-[1000px]">
                ...
                <TableBody>
                  {formData.vesData.map((row: any, i: number) => (
                    <TableRow key={i} className={cn("h-11 border-slate-100 transition-colors", i % 2 === 0 ? "bg-white" : "bg-slate-50/30")}>
        ...
        <CardContent className="p-10 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div className="space-y-4">
              ...
              <Select disabled={!isAllowed} onValueChange={(v) => {updateField('recommendationType', v); setIsRecommendationDialogOpen(true);}} value={formData.recommendationType || ''}>
                <SelectTrigger className="h-14 border-slate-200 rounded-2xl font-black uppercase text-xs tracking-widest shadow-sm"><SelectValue placeholder="SELECT RECOMMENDATION" /></SelectTrigger>
                <SelectContent className="rounded-2xl border-slate-200">{recommendationTypeOptions.map(o => <SelectItem key={o.value} value={o.value} className="text-xs font-black uppercase">{o.label}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-4">
               ...
               <div className="grid grid-cols-2 gap-4">
                  <Select onValueChange={(val) => handleNearbyTypeSelect('borewell', val)} disabled={!isAllowed}>
                    <SelectTrigger className="h-12"><SelectValue placeholder="Borewell Status" /></SelectTrigger>
                    <SelectContent><SelectItem value="borewell1">Add Bore well-1</SelectItem><SelectItem value="borewell2">Add Bore well-2</SelectItem><SelectItem value="borewell3">Add Bore well-3</SelectItem><SelectItem value="none">None found</SelectItem></SelectContent>
                  </Select>
                  <Select onValueChange={(val) => handleNearbyTypeSelect('openwell', val)} disabled={!isAllowed}>
                    <SelectTrigger className="h-12"><SelectValue placeholder="Openwell Status" /></SelectTrigger>
                    <SelectContent><SelectItem value="openwell1">Add open well-1</SelectItem><SelectItem value="openwell2">Add open well-2</SelectItem><SelectItem value="openwell3">Add open well-3</SelectItem><SelectItem value="none">None found</SelectItem></SelectContent>
                  </Select>
               </div>
            </div>
          </div>
...
const RecommendationDialog = ({isOpen, onOpenChange, formData, updateField}: any) => (
  <Dialog open={isOpen} onOpenChange={onOpenChange}>
    <DialogContent className="max-w-xl">
      ...
      {(formData.recommendationType === 'borewell' || formData.recommendationType === 'tubewell' || formData.recommendationType === 'filterpoint') && (
        <div className="space-y-4 py-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormFieldItem label="Diameter" id="recBorewellDiameter"><Select onValueChange={v=>updateField('recBorewellDiameter', v)} value={formData.recBorewellDiameter}><SelectTrigger/><SelectContent>{borewellDiameterOptions.map(o=><SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}</SelectContent></Select></FormFieldItem>
          </div>
      ...
      {formData.recommendationType === 'openwell' && (
        <div className="space-y-4 py-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormFieldItem label="Diameter (m)" id="recOpenwellDiameter"><Select onValueChange={v=>updateField('recOpenwellDiameter', v)} value={formData.recOpenwellDiameter}><SelectTrigger/><SelectContent>{openwellDiameterOptions.map(o=><SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}</SelectContent></Select></FormFieldItem>
          </div>
      ...
      <DialogFooter><Button onClick={() => onOpenChange(false)} className="w-full h-12 rounded-xl">Confirm Parameters</Button></DialogFooter>
    </DialogContent>
  </Dialog>
);
        ...
        <div className="space-y-4 py-4">
         ...
         <div className="space-y-4 py-4">
          ...
          <FormFieldItem label="Type" id={`nod_type${index}`}>
            <Select onValueChange={v=>updateField(`nearbyOpenwell${index}Type`, v)} value={formData[`nearbyOpenwell${index}Type`]}><SelectTrigger/><SelectContent><SelectItem value="Perennial">Perennial</SelectItem><SelectItem value="Seasonal">Seasonal</SelectItem></SelectContent></Select>
          </FormFieldItem>
        </div>
      )}
      <DialogFooter><Button onClick={() => onOpenChange(false)} className="w-full h-12 rounded-xl">Save Details</Button></DialogFooter>
    </DialogContent>
  </Dialog>
)};
